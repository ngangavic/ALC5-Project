...
class SkillsRecyclerViewAdapter(private val skills: List<SkillIqModel>) : RecyclerView.Adapter<SkillHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SkillHolder {
        val viewHolder: SkillHolder

        val layoutView =
            LayoutInflater.from(parent.context).inflate(R.layout.row_skill_iq_leaders, parent, false)
    ...
    override fun onBindViewHolder(holder: SkillHolder, position: Int) {
        holder.textViewHours.text=skills[position].score+" learning hours,"+skills[position].country
